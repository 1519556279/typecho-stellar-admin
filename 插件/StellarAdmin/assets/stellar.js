...
(function () {
    ...
    function toast(msg, type) {
        ...
        toastTimer = setTimeout(function () { t.style.opacity = '0'; setTimeout(function () { t.remove(); }, 300); }, 2600);
    }
    ...
    function bindEditorTools() {
        ...
        if (guide) {
            guide.addEventListener('click', function (e) {
                ...
                ta.focus();
                ta.dispatchEvent(new Event('input'));
                /* 插入后自动收起语法面板（用户一般只点一次，不必手动关闭） */
                guide.hidden = true;
                if (mdToggle) mdToggle.setAttribute('data-on', '0');
